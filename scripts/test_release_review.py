...
All trial IDs/dates/content below are synthetic test fixtures, not research news.
...
from research_contract import source_record
...
def change(program='MODIF-MS / ifenprodil', after='RECRUITING', source='ClinicalTrials.gov'):
    ids={'MODIF-MS / ifenprodil':'NCT06330077','ReVIVE':'NCT05359653','CCMR Two':'NCT05131828'}
    rid=ids.get(program,'NCT09999998')
    url=f'https://clinicaltrials.gov/study/{rid}'
    obs={'id':rid,'url':url,'status':after,'observation_valid':True}
    if source=='EU CTIS':
        rid='2023-507874-42-00';url=f'https://euclinicaltrials.eu/ctis-public/view/{rid}?lang=en'
        obs.update(id=rid,url=url,official_url=url,official_status=after)
    obs['provenance']=source_record(rid,url,{'status':after})
    obs['provenance']['fetched_at']=WHEN
    return mr.change(program,source,'Human trial','Primary registry','status',
        'NOT_YET_RECRUITING',after,url,'Synthetic operational-status fixture; not an efficacy result.',observation=obs)


def browser(payload, expression):
    # Evaluate the REAL app.js. Leave startup fetch pending so rendering/network
    # is not performed. Pure transformation functions are called explicitly.
    script = r'''
...
class ReleaseReview(unittest.TestCase):
    ...
    def test_control_normal_status_event_and_idempotent_replay(self):
        feed = empty_feed()
        report = {'generated_at':WHEN,'substantive_changes':[change()]}
    ...
    def test_canonical_identity_is_preserved_in_public_programme(self):
        c=collect(); c.update(review_confidence(c))
        p=pub.programme_from_candidate(c)
        self.assertEqual(p['canonical_id'],c['canonical_program_id'])

    def test_actual_modif_card_receives_its_status_override(self):
        feed=empty_feed(); pub.apply_daily(feed,{'generated_at':WHEN,'substantive_changes':[change(after='Ongoing, recruiting', source='EU CTIS')]})
    ...
    def test_live_added_programme_receives_subsequent_status_override(self):
        ...
        result=browser({'base':self.base,'feed':feed,'name':name},
            'mergeLiveData(input.base,input.feed).pipeline.find(x=>x.name===input.name).status')
        self.assertTrue(result['ok'],result)
    ...
    def test_registry_status_does_not_destroy_existing_readout(self):
        ...
        self.assertTrue(result['ok'],result)
        self.assertIn(old,json.dumps(result['value'],ensure_ascii=False), 'Operational status erased the scientific readout summary')
    ...
    def test_ctis_network_failure_does_not_publish_halt_lift(self):
        ...
        try:
            ...
            return  # Raising a source failure instead of inventing a snapshot is valid.
    ...
    def test_ctis_conflicting_mirror_cannot_override_official_status(self):
        ...
        with patch.object(mr,'fetch',side_effect=fetch):
            new=mr.ctis_snapshot('2025-522922-11-00')
    ...
    def test_greek_site_recruitment_change_detected_without_count_change(self):
        api={'protocolSection':{'identificationModule':{'nctId':'NCT09999998','briefTitle':'RX-TEST'},'statusModule':{'overallStatus':'RECRUITING'},
    ...
    def test_netlify_uncached_build_is_not_skipped_when_refs_are_equal(self):
        # Netlify documents CACHED_COMMIT_REF == COMMIT_REF for an uncached build.
        run=subprocess.run(['node','scripts/netlify_ignore.js'],cwd=ROOT,
            env={**os.environ,'CACHED_COMMIT_REF':'HEAD','COMMIT_REF':'HEAD'},
    ...
    def test_rejected_record_can_be_reconsidered_when_its_evidence_changes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, ExitStack() as stack:
            ...
            dd.main()
            first=json.loads(report.read_text())['candidates']
            ...
            self.assertTrue(issue_worthy(improved))
            scanner.return_value=[improved]
            dd.main()
